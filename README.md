# Arena Allocator
Single-header arena allocator. C23.
...
This project targets C23 and uses modern features such as `nullptr`,
`constexpr`, `auto`, standard attributes, and `alignof`. Older C
...
The code was written to build with any compiler that supports the C23 standard
and run on any platform. However, there are some issues with building on
Windows with the `justfile` (via `just`). Please read all of the below information.
...
**NOTE** The `justfile` currently has not been configured to work on Windows when running `$ just test` due to the use of valgrind and the absense of `.exe` extensions. This should be a simple fix and is on my TODO list (feel free to open an issue and fix this yourself!).
