...
//   source file, include the header with `DEFINE_RKTEST_IMPLEMENTATION`
//   defined:
//
//      // rktest.c
...
//
//       gcc -lm rktest.c factorial.c factorial_tests.c -Irktest/include -o
//       unit_tests
//
// ASSERTIONS
//
...
//   ----------------------------------- |
//   ------------------------------------------------------------- | |
//   EXPECT_STREQ(actual, expected)      | `actual` is the same string as
//   `expected`                     | | EXPECT_STRNE(actual, expected)      |
//   `actual` is NOT the same string as `expected`                 | |
//   EXPECT_CASE_STREQ(actual, expected) | `actual` is the same string as
//   `expected` (ignoring case)     | | EXPECT_CASE_STRNE(actual, expected) |
//   `actual` is NOT the same string as `expected` (ignoring case) | |
//   EXPECT_CHAR_EQ(actual, expected)    | `actual` is the same ascii character
//   as `expected`            |
//
...
//   ---------------------------------- |
//   ------------------------------------------------------ | |
//   EXPECT_FLOAT_EQ(actual, expected)  | `actual` and `expected` are within 4
//   ULP of each other | | EXPECT_DOUBLE_EQ(actual, expected) | `actual` and
//   `expected` are within 4 ULP of each other |
//
//   NOTE: See
...
//        Run only the tests that matches the globbing pattern. * matches
//        against any number of characters, and ? matches any single character.
//
//      --rktest_print_time=0
...
#define TEST(SUITE, NAME)                                                      \
  ...
  const rktest_test_t SUITE##_##NAME##_data = {.suite_name = #SUITE,           \
                                               .test_name = #NAME,             \
                                               .run = &SUITE##_##NAME##_impl}; \
  ADD_TO_MEMORY_SECTION_BEGIN                                                  \
  const rktest_test_t *const SUITE##_##NAME##_data##_##ptr =                   \
      &SUITE##_##NAME##_data;                                                  \
...
#define TEST_SETUP(SUITE)                                                      \
  ...
  const rktest_test_t SUITE##_##setup##_data = {.suite_name = #SUITE,          \
                                                .setup = &SUITE##_##setup};    \
  ADD_TO_MEMORY_SECTION_BEGIN                                                  \
  const rktest_test_t *const SUITE##_##setup##_data##_##ptr =                  \
      &SUITE##_setup_data;                                                     \
  ADD_TO_MEMORY_SECTION_END                                                    \
...
#define TEST_TEARDOWN(SUITE)                                                   \
  ...
  const rktest_test_t SUITE##_##teardown##_data = {                            \
      .suite_name = #SUITE, .teardown = &SUITE##_##teardown};                  \
  ADD_TO_MEMORY_SECTION_BEGIN                                                  \
  const rktest_test_t *const SUITE##_##teardown##_data##_##ptr =               \
      &SUITE##_teardown_data;                                                  \
  ADD_TO_MEMORY_SECTION_END                                                    \
  void SUITE##_teardown(void)

/* Bool checks */
#define EXPECT_TRUE(expr)                                                      \
  RKTEST_CHECK_BOOL(expr, true, RKTEST_CHECK_EXPECT, " ")
#define EXPECT_FALSE(lhs)                                                      \
  RKTEST_CHECK_BOOL(lhs, false, RKTEST_CHECK_EXPECT, " ")

#define ASSERT_TRUE(expr)                                                      \
  RKTEST_CHECK_BOOL(expr, true, RKTEST_CHECK_ASSERT, " ")
#define ASSERT_FALSE(lhs)                                                      \
  RKTEST_CHECK_BOOL(lhs, false, RKTEST_CHECK_ASSERT, " ")

#define EXPECT_TRUE_INFO(expr, ...)                                            \
  RKTEST_CHECK_BOOL(expr, true, RKTEST_CHECK_EXPECT, __VA_ARGS__)
#define EXPECT_FALSE_INFO(lhs, ...)                                            \
  RKTEST_CHECK_BOOL(lhs, false, RKTEST_CHECK_EXPECT, __VA_ARGS__)

#define ASSERT_TRUE_INFO(expr, ...)                                            \
  RKTEST_CHECK_BOOL(expr, true, RKTEST_CHECK_ASSERT, __VA_ARGS__)
#define ASSERT_FALSE_INFO(lhs, ...)                                            \
  RKTEST_CHECK_BOOL(lhs, false, RKTEST_CHECK_ASSERT, __VA_ARGS__)

/* Integer checks */
#define EXPECT_EQ(lhs, rhs)                                                    \
  RKTEST_CHECK_EQ(int, "%d", lhs, rhs, RKTEST_CHECK_EXPECT, " ")
#define EXPECT_NE(lhs, rhs)                                                    \
  RKTEST_CHECK_CMP(int, "%d", lhs, rhs, !=, RKTEST_CHECK_EXPECT, " ")
...
#define ASSERT_EQ(lhs, rhs)                                                    \
  RKTEST_CHECK_EQ(int, "%d", lhs, rhs, RKTEST_CHECK_ASSERT, " ")
#define ASSERT_NE(lhs, rhs)                                                    \
  RKTEST_CHECK_CMP(int, "%d", lhs, rhs, !=, RKTEST_CHECK_ASSERT, " ")
...
#define EXPECT_EQ_INFO(lhs, rhs, ...)                                          \
  RKTEST_CHECK_EQ(int, "%d", lhs, rhs, RKTEST_CHECK_EXPECT, __VA_ARGS__)
#define EXPECT_NE_INFO(lhs, rhs, ...)                                          \
  RKTEST_CHECK_CMP(int, "%d", lhs, rhs, !=, RKTEST_CHECK_EXPECT, __VA_ARGS__)
#define EXPECT_LT_INFO(lhs, rhs, ...)                                          \
  RKTEST_CHECK_CMP(int, "%d", lhs, rhs, <, RKTEST_CHECK_EXPECT, __VA_ARGS__)
#define EXPECT_LE_INFO(lhs, rhs, ...)                                          \
  RKTEST_CHECK_CMP(int, "%d", lhs, rhs, <=, RKTEST_CHECK_EXPECT, __VA_ARGS__)
#define EXPECT_GT_INFO(lhs, rhs, ...)                                          \
  RKTEST_CHECK_CMP(int, "%d", lhs, rhs, >, RKTEST_CHECK_EXPECT, __VA_ARGS__)
#define EXPECT_GE_INFO(lhs, rhs, ...)                                          \
  RKTEST_CHECK_CMP(int, "%d", lhs, rhs, >=, RKTEST_CHECK_EXPECT, __VA_ARGS__)

#define ASSERT_EQ_INFO(lhs, rhs, ...)                                          \
  RKTEST_CHECK_EQ(int, "%d", lhs, rhs, RKTEST_CHECK_ASSERT, __VA_ARGS__)
#define ASSERT_NE_INFO(lhs, rhs, ...)                                          \
  RKTEST_CHECK_CMP(int, "%d", lhs, rhs, !=, RKTEST_CHECK_ASSERT, __VA_ARGS__)
#define ASSERT_LT_INFO(lhs, rhs, ...)                                          \
  RKTEST_CHECK_CMP(int, "%d", lhs, rhs, <, RKTEST_CHECK_ASSERT, __VA_ARGS__)
#define ASSERT_LE_INFO(lhs, rhs, ...)                                          \
  RKTEST_CHECK_CMP(int, "%d", lhs, rhs, <=, RKTEST_CHECK_ASSERT, __VA_ARGS__)
#define ASSERT_GT_INFO(lhs, rhs, ...)                                          \
  RKTEST_CHECK_CMP(int, "%d", lhs, rhs, >, RKTEST_CHECK_ASSERT, __VA_ARGS__)
#define ASSERT_GE_INFO(lhs, rhs, ...)                                          \
  RKTEST_CHECK_CMP(int, "%d", lhs, rhs, >=, RKTEST_CHECK_ASSERT, __VA_ARGS__)

/* Long checks */
#define EXPECT_LONG_EQ(lhs, rhs)                                               \
  RKTEST_CHECK_EQ(long, "%ld", lhs, rhs, RKTEST_CHECK_EXPECT, " ")
#define EXPECT_LONG_NE(lhs, rhs)                                               \
  RKTEST_CHECK_CMP(long, "%ld", lhs, rhs, !=, RKTEST_CHECK_EXPECT, " ")
#define EXPECT_LONG_LT(lhs, rhs)                                               \
  RKTEST_CHECK_CMP(long, "%ld", lhs, rhs, <, RKTEST_CHECK_EXPECT, " ")
#define EXPECT_LONG_LE(lhs, rhs)                                               \
  RKTEST_CHECK_CMP(long, "%ld", lhs, rhs, <=, RKTEST_CHECK_EXPECT, " ")
#define EXPECT_LONG_GT(lhs, rhs)                                               \
  RKTEST_CHECK_CMP(long, "%ld", lhs, rhs, >, RKTEST_CHECK_EXPECT, " ")
#define EXPECT_LONG_GE(lhs, rhs)                                               \
  RKTEST_CHECK_CMP(long, "%ld", lhs, rhs, >=, RKTEST_CHECK_EXPECT, " ")

#define ASSERT_LONG_EQ(lhs, rhs)                                               \
  RKTEST_CHECK_EQ(long, "%ld", lhs, rhs, RKTEST_CHECK_ASSERT, " ")
#define ASSERT_LONG_NE(lhs, rhs)                                               \
  RKTEST_CHECK_CMP(long, "%ld", lhs, rhs, !=, RKTEST_CHECK_ASSERT, " ")
#define ASSERT_LONG_LT(lhs, rhs)                                               \
  RKTEST_CHECK_CMP(long, "%ld", lhs, rhs, <, RKTEST_CHECK_ASSERT, " ")
#define ASSERT_LONG_LE(lhs, rhs)                                               \
  RKTEST_CHECK_CMP(long, "%ld", lhs, rhs, <=, RKTEST_CHECK_ASSERT, " ")
#define ASSERT_LONG_GT(lhs, rhs)                                               \
  RKTEST_CHECK_CMP(long, "%ld", lhs, rhs, >, RKTEST_CHECK_ASSERT, " ")
#define ASSERT_LONG_GE(lhs, rhs)                                               \
  RKTEST_CHECK_CMP(long, "%ld", lhs, rhs, >=, RKTEST_CHECK_ASSERT, " ")

#define EXPECT_LONG_EQ_INFO(lhs, rhs, ...)                                     \
  RKTEST_CHECK_EQ(long, "%ld", lhs, rhs, RKTEST_CHECK_EXPECT, __VA_ARGS__)
#define EXPECT_LONG_NE_INFO(lhs, rhs, ...)                                     \
  RKTEST_CHECK_CMP(long, "%ld", lhs, rhs, !=, RKTEST_CHECK_EXPECT, __VA_ARGS__)
#define EXPECT_LONG_LT_INFO(lhs, rhs, ...)                                     \
  RKTEST_CHECK_CMP(long, "%ld", lhs, rhs, <, RKTEST_CHECK_EXPECT, __VA_ARGS__)
#define EXPECT_LONG_LE_INFO(lhs, rhs, ...)                                     \
  RKTEST_CHECK_CMP(long, "%ld", lhs, rhs, <=, RKTEST_CHECK_EXPECT, __VA_ARGS__)
#define EXPECT_LONG_GT_INFO(lhs, rhs, ...)                                     \
  RKTEST_CHECK_CMP(long, "%ld", lhs, rhs, >, RKTEST_CHECK_EXPECT, __VA_ARGS__)
#define EXPECT_LONG_GE_INFO(lhs, rhs, ...)                                     \
  RKTEST_CHECK_CMP(long, "%ld", lhs, rhs, >=, RKTEST_CHECK_EXPECT, __VA_ARGS__)

#define ASSERT_LONG_EQ_INFO(lhs, rhs, ...)                                     \
  RKTEST_CHECK_EQ(long, "%ld", lhs, rhs, RKTEST_CHECK_ASSERT, __VA_ARGS__)
#define ASSERT_LONG_NE_INFO(lhs, rhs, ...)                                     \
  RKTEST_CHECK_CMP(long, "%ld", lhs, rhs, !=, RKTEST_CHECK_ASSERT, __VA_ARGS__)
#define ASSERT_LONG_LT_INFO(lhs, rhs, ...)                                     \
  RKTEST_CHECK_CMP(long, "%ld", lhs, rhs, <, RKTEST_CHECK_ASSERT, __VA_ARGS__)
#define ASSERT_LONG_LE_INFO(lhs, rhs, ...)                                     \
  RKTEST_CHECK_CMP(long, "%ld", lhs, rhs, <=, RKTEST_CHECK_ASSERT, __VA_ARGS__)
#define ASSERT_LONG_GT_INFO(lhs, rhs, ...)                                     \
  RKTEST_CHECK_CMP(long, "%ld", lhs, rhs, >, RKTEST_CHECK_ASSERT, __VA_ARGS__)
#define ASSERT_LONG_GE_INFO(lhs, rhs, ...)                                     \
  RKTEST_CHECK_CMP(long, "%ld", lhs, rhs, >=, RKTEST_CHECK_ASSERT, __VA_ARGS__)
...
#define EXPECT_FLOAT_EQ(lhs, rhs)                                              \
  RKTEST_CHECK_FLOAT_EQ(float, lhs, rhs, rktest_floats_within_4_ulp,           \
                        RKTEST_CHECK_EXPECT, " ")
#define EXPECT_FLOAT_EQ_INFO(lhs, rhs, ...)                                    \
  RKTEST_CHECK_FLOAT_EQ(float, lhs, rhs, rktest_floats_within_4_ulp,           \
                        RKTEST_CHECK_EXPECT, __VA_ARGS__)
...
#define ASSERT_FLOAT_EQ_INFO(lhs, rhs, ...)                                    \
  RKTEST_CHECK_FLOAT_EQ(float, lhs, rhs, rktest_floats_within_4_ulp,           \
                        RKTEST_CHECK_ASSERT, __VA_ARGS__)

#define EXPECT_DOUBLE_EQ(lhs, rhs)                                             \
  RKTEST_CHECK_FLOAT_EQ(double, lhs, rhs, rktest_doubles_within_4_ulp,         \
                        RKTEST_CHECK_EXPECT, " ")
...
#define EXPECT_STREQ(lhs, rhs)                                                 \
  RKTEST_CHECK_STREQ(lhs, rhs, RKTEST_CHECK_EXPECT, RKTEST_MATCH_CASE, " ")
#define EXPECT_STRNE(lhs, rhs)                                                 \
  RKTEST_CHECK_STRNE(lhs, rhs, RKTEST_CHECK_EXPECT, RKTEST_MATCH_CASE, " ")
#define EXPECT_CASE_STREQ(lhs, rhs)                                            \
  RKTEST_CHECK_STREQ(lhs, rhs, RKTEST_CHECK_EXPECT, RKTEST_CASE_INSENSETIVE,   \
                     " ")
#define EXPECT_CASE_STRNE(lhs, rhs)                                            \
  RKTEST_CHECK_STRNE(lhs, rhs, RKTEST_CHECK_EXPECT, RKTEST_CASE_INSENSETIVE,   \
                     " ")
#define EXPECT_CHAR_EQ(lhs, rhs)                                               \
  RKTEST_CHECK_CHAR_EQ(lhs, rhs, RKTEST_CHECK_EXPECT, " ")
...
#define EXPECT_CASE_STRNE_INFO(lhs, rhs, ...)                                  \
  RKTEST_CHECK_STRNE(lhs, rhs, RKTEST_CHECK_EXPECT, RKTEST_CASE_INSENSETIVE,   \
                     __VA_ARGS__)
#define EXPECT_CHAR_EQ_INFO(lhs, rhs, ...)                                     \
  RKTEST_CHECK_CHAR_EQ(lhs, rhs, RKTEST_CHECK_EXPECT, __VA_ARGS__)

#define ASSERT_STREQ(lhs, rhs)                                                 \
  RKTEST_CHECK_STREQ(lhs, rhs, RKTEST_CHECK_ASSERT, RKTEST_MATCH_CASE, " ")
#define ASSERT_STRNE(lhs, rhs)                                                 \
  RKTEST_CHECK_STRNE(lhs, rhs, RKTEST_CHECK_ASSERT, RKTEST_MATCH_CASE, " ")
#define ASSERT_CASE_STREQ(lhs, rhs)                                            \
  RKTEST_CHECK_STREQ(lhs, rhs, RKTEST_CHECK_ASSERT, RKTEST_CASE_INSENSETIVE,   \
                     " ")
#define ASSERT_CASE_STRNE(lhs, rhs)                                            \
  RKTEST_CHECK_STRNE(lhs, rhs, RKTEST_CHECK_ASSERT, RKTEST_CASE_INSENSETIVE,   \
                     " ")
#define ASSERT_CHAR_EQ(lhs, rhs)                                               \
  RKTEST_CHECK_CHAR_EQ(lhs, rhs, RKTEST_CHECK_ASSERT, " ")
...
#define ASSERT_CASE_STRNE_INFO(lhs, rhs, ...)                                  \
  RKTEST_CHECK_STRNE(lhs, rhs, RKTEST_CHECK_ASSERT, RKTEST_CASE_INSENSETIVE,   \
                     __VA_ARGS__)
#define ASSERT_CHAR_EQ_INFO(lhs, rhs, ...)                                     \
  RKTEST_CHECK_CHAR_EQ(lhs, rhs, RKTEST_CHECK_ASSERT, __VA_ARGS__)
...
#define RKTEST_CHECK_STREQ(lhs, rhs, is_assert, match_case, ...)               \
  do {                                                                         \
    ...
    if (match_case ? (strcmp(lhs_val, rhs_val) != 0)                           \
                   : (rktest_strcasecmp(lhs_val, rhs_val) != 0)) {             \
      if (rktest_filenames_enabled()) {                                        \
        printf("%s(%d): ", __FILE__, __LINE__);                                \
      }                                                                        \
      printf("error: Expected equality of these values:\n");                   \
...
#define vec_new() NULL
#define vec_free(vec)                                                          \
  ((void)((vec) ? free(vec_header(vec)) : (void)0), (vec) = NULL)
#define vec_push(vec, ...)                                                     \
  (vec_maybegrow(vec, 1), (vec)[vec_header(vec)->length++] = (__VA_ARGS__))
...
#define vec_maybegrow(vec, n)                                                  \
  ((!(vec) || vec_header(vec)->length + (n) > vec_header(vec)->capacity)       \
       ? (vec_grow(vec, n, 0), 0)                                              \
       : 0)
#define vec_header(t) ((rk_vector_header_t *)(t) - 1)
#define vec_grow(vec, b, c) ((vec) = vec_growf((vec), sizeof *(vec), (b), (c)))

static void *vec_growf(void *vec, size_t elem_size, size_t addlen,
                       size_t min_cap) {
  void *new_vec;
  ...
  new_vec = realloc((vec) ? vec_header(vec) : 0,
                    elem_size * min_cap + sizeof(rk_vector_header_t));
...
rktest_millis_t rktest_timer_stop(rktest_timer_t *timer) {
  ...
  rktest_millis_t ms = (rktest_millis_t)((timer->end - timer->start) *
                                         timer->timebase_info.numer /
                                         timer->timebase_info.denom / 1000000);
  return ms;
}
#else
rktest_millis_t rktest_timer_stop(rktest_timer_t *timer) {
  ...
  ms += (rktest_millis_t)((timer->end.tv_sec - timer->start.tv_sec) *
                          1000.0); // seconds
  ms += (rktest_millis_t)((timer->end.tv_nsec - timer->start.tv_nsec) /
                          1000.0); // milliseconds
  return ms;
}
#endif
...
// https://christophercrouzet.com/blog/dev/rexo-part-2
#if defined(_MSC_VER)
__pragma(section("rktest$begin", read));
__pragma(section("rktest$data", read));
__pragma(section("rktest$end", read));
__declspec(allocate(
    "rktest$begin")) extern const rktest_test_t *const test_data_begin = NULL;
__declspec(allocate(
    "rktest$end")) extern const rktest_test_t *const test_data_end = NULL;
#elif defined(__APPLE__)
extern const rktest_test_t *const
    __start_rktest __asm("section$start$__DATA$rktest");
extern const rktest_test_t *const
    __stop_rktest __asm("section$end$__DATA$rktest");
__attribute__((used, section("__DATA,rktest")))
const rktest_test_t *const dummy = NULL;
#elif defined(__unix__)
extern const rktest_test_t *const __start_rktest;
extern const rktest_test_t *const __stop_rktest;
__attribute__((used, section("rktest"))) const rktest_test_t *const dummy =
    NULL;
#endif
...
bool rktest_colors_enabled(void) { return g_colors_enabled; }

bool rktest_filenames_enabled(void) { return g_filenames_enabled; }

void rktest_fail_current_test(void) { g_current_test_failed = true; }
...
static void print_usage(void) {
  ...
  printf("    Run only the tests that matches the globbing pattern. * matches "
         "against\n");
  printf("    any number of characters, and ? matches any single character.\n");
  printf("\n");
  printf("  --rktest_print_time=0\n");
  printf("    Disable printing out the elapsed time for test cases and test "
         "suites.\n");
  printf("\n");
  printf("  --rktest_print_filenames=0\n");
  printf("    Disable printing out the filename of a test case on assert "
         "failure.\n");
}
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (strcmp(arg, "--help") == 0 || strcmp(arg, "-h") == 0) {
      ...
      if (strlen(filter_pattern) > RKTEST_MAX_FILTER_LENGTH - 1) {
        fprintf(stderr, "Error: filter pattern too long. Max length is (%d)",
                RKTEST_MAX_FILTER_LENGTH - 1);
...
static rktest_config_t initialize(int argc, const char *argv[]) {
  ...
  if (g_colors_enabled) {
    ...
    case RKTEST_ENABLE_VTERM_ERROR_ENABLE_VIRTUAL_TERMINAL_FAILED:
      g_colors_enabled = false;
      fprintf(stderr, "Error: SetConsoleMode to "
                      "ENABLE_VIRTUAL_TERMINAL_PROCESSING failed\n");
      break;
...
static rktest_suite_t *find_suite_with_name(vec_t(rktest_suite_t) suites,
                                            const char *suite_name) {
...
static bool test_matches_filter(const rktest_test_t *test,
                                const char *pattern) {
  if (*pattern == '\0') {
    return true;
  }
  ...
  snprintf(full_test_name, sizeof(full_test_name) / sizeof(char), "%s.%s",
...
static rktest_environment_t setup_test_env(const rktest_config_t *config) {
  ...
  for (const rktest_test_t *const *it = TEST_DATA_BEGIN; it != TEST_DATA_END;
       it++) {
    if (*it == NULL) {
      continue;
    }
    ...
    rktest_suite_t *suite =
        find_suite_with_name(env.test_suites, test.suite_name);
    if (!suite) {
...
static rktest_report_t run_all_tests(rktest_environment_t *env,
  ...
  vec_foreach(rktest_suite_t *, suite, env->test_suites) {
    ...
    const size_t num_filtered_tests =
        vec_len(suite->tests) - suite->num_disabled_tests;
    ...
    rktest_log_info("[----------] ", "%zu tests from %s ", num_filtered_tests,
                    suite->name);
    if (config->print_timestamps_enabled) {
...
static void print_failed_tests(rktest_report_t *report) {
  ...
  printf(" %zu FAILED TEST%s\n", vec_len(report->failed_tests),
         vec_len(report->failed_tests) > 1 ? "S" : "");
}
...
int rktest_main(int argc, const char *argv[]) {
  ...
  rktest_log_info("[==========] ", "%zu tests from %zu test suites ran. ",
                  env.total_num_filtered_tests, env.total_num_filtered_suites);
  if (config.print_timestamps_enabled) {
  ...
  if (env.total_num_disabled_tests > 0) {
    ...
    rktest_printf_yellow("  YOU HAVE %zu DISABLED TEST%s\n",
                         env.total_num_disabled_tests,
                         env.total_num_disabled_tests > 1 ? "S" : "");
  }
